...
function TrendBadge({ value }) {
  ...
  const color = value >= 5 ? "text-accent-green" : value >= 1 ? "text-accent-amber" : "text-gray-500";
  ...
  if (!checks.length) {
    return (
      <div className="card p-8 text-center">
        <p className="text-gray-500 text-sm">No check history yet. The cron job runs every 10 minutes.</p>
      </div>
    );
  }
