  ...
  if (loading) {
    return (
      <div className="card overflow-hidden">
        <div className="p-4 border-b border-surface-border"><div className="h-5 w-40 bg-surface-border rounded animate-pulse" /></div>
        {[...Array(5)].map((_, i) => (
          <div key={i} className="px-4 py-3 border-b border-surface-border flex gap-4">
            {[...Array(3)].map((_, j) => <div key={j} className="h-4 flex-1 bg-surface-border rounded animate-pulse" />)}
          </div>
        ))}
      </div>
    );
  }
  if (!entries.length) return <div className="card p-8 text-center"><p className="text-gray-500 text-sm">No data yet. Check back shortly.</p></div>;
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const [recentRes, hourlyRes] = await Promise.all([
        getPublisherRecent(20, stateFilter),
        getPublisherHourly(hourlyDays, stateFilter),
      ]);
      setRecentChecks(recentRes.data || []);
      setHourlyAverages(hourlyRes.data || []);
      setAutoRefreshSecs(AUTO_REFRESH);
      showToast("Data refreshed from CRM.");
    } catch { showToast("Refresh failed. Try again.", "error"); }
    finally { setRefreshing(false); }
  };

  const checkedAt = snapshot?.checkedAt ? new Date(snapshot.checkedAt).toLocaleString() : null;
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-md bg-accent-green flex items-center justify-center shrink-0">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0d0f14" strokeWidth="2.5"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleRefresh} disabled={refreshing} className="btn-primary py-1.5 px-3 text-xs flex items-center gap-1.5">
              {refreshing ? (
                <><div className="w-3 h-3 border-2 border-surface/40 border-t-surface rounded-full animate-spin" />Refreshing...</>
              ) : (
                <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="23 4 23 10 17 10" /><polyline points="1 20 1 14 7 14" /><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" /></svg>Refresh Now</>
              )}
            </button>
            <a href={getPublisherDownloadUrl()} target="_blank" rel="noopener noreferrer" className="btn-excel py-1.5 px-3 text-xs flex items-center gap-1.5">
            ...
            <div className="flex gap-1.5 flex-wrap">
              <button
                ...
                className={`px-3 py-1 rounded text-xs font-display transition-colors ${
                  stateFilter === "" ? "bg-surface-border text-white font-semibold" : "text-gray-500 hover:text-gray-300"
                }`}
              >
                All States
              </button>
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 px-4 py-3 rounded-xl text-sm font-medium shadow-xl animate-slide-up ${
          toast.type === "error" ? "bg-red-500/20 border border-red-500/30 text-red-300" : "bg-accent-green/20 border border-accent-green/30 text-accent-green"
        }`}>
          {toast.msg}
        </div>
      )}
